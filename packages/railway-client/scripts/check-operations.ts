 ...
 * `src/documents.ts` must match the `.graphql` files beside them.
 *
...
/**
 * Resolved from this module, never from the cwd: Turborepo runs each package
 * with the package directory as the working directory, and the schema excerpt
 * is research evidence that stays at the repository root.
 */
const SCHEMA_PATH = fileURLToPath(
  new URL('../../../openspec/_research/railway-schema-excerpt.graphql', import.meta.url),
);
...
export function check(): string[] {
  ...
  for (const file of files) {
    ...
    if (!inlined.has(normalise(source))) {
      problems.push(
        `${file}: no matching export in src/documents.ts — ` +
  ...
  if (problems.length === 0) {
    console.log(
      `Operation check passed: ${files.length} document(s) against the schema excerpt ` +
