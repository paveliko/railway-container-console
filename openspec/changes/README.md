...
| [`container-verbs/`](container-verbs/) | proposed · implemented | `up()` / `down()`, the port's verbs, the mutation documents. It was blocked on `Q-API-2` — the one change whose body an agent could not write until the owner picked — and `D-API-5` picked, ratified 2026-09-14: stop ↔ restart, falling back to `serviceInstanceDeployV2` for the first up. `vite-console` wrote the body against that pair. |
