...
- [x] **T-CV-3 The verbs, fully covered.** Result: the branch and the guard, per design §5 — `verbs.ts` in `@repo/railway-client`, the claim in `Poller`, the `409` in `routes.ts`. *(The Result originally named `createActions` in `packages/container-core/src/actions.ts`, which was not built — design §5 records why.)* Written by `vite-console` T-VC-2.1 / T-VC-2.2. Depends on: T-CV-1. Acceptance: V-CV-1 … V-CV-7, V-CV-9. **Closed 2026-09-15:** `verbs.test.ts` gains a `REMOVED` fixture, so `V-CV-3` — the last criterion here with no run — is covered and the deploy branch is tested rather than inferred from the restart branch passing. Verified by: `pnpm turbo run test --filter=@repo/railway-client`, plus the poller and route cases named in `verification.md`.
