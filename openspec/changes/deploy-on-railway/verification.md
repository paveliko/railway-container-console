...
- V-DR-7 `manual` — Design §1 records, with source and date, that Config as Code is deprecated and closed to new services, and what `.railway/railway.ts` can express; and design §2 gives every settings row its disposition under each live mechanism, marking `[to-verify]` where the reference shows no option. *This is verified by reading the design section the same task wrote — it is a record of what Railway's documentation says, not a check against a Railway service.*
- V-DR-8 `manual` — After linking, the console service shows the build command, start command, watch paths and restart policy of design §2, by whichever mechanism `Q-OPS-8` picked.
