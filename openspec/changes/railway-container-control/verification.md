...
- V-40 `build` — A `.graphql` file containing `mutation` is imported only from `container/actions.ts`; any other importer fails the build (lint rule or grep). **Restated for packages** (`D-OPS-2` ratified 2026-09-14) as `V-MW-10` — documents live only in `@repo/railway-client` — and `V-CV-9`. This line is now verified in its restated form only. **Amended by `vite-console`, recorded 2026-09-15:** the restatement's second half named `container-core/actions.ts`, a file that was never written — `container-verbs` design §5 records what was built instead and why. `V-CV-9` now names the boundary that exists: the provider's `up()` / `down()` are called from `apps/console/src/server/routes.ts` alone, after the passphrase gate and the guard claim.
